...
function getgeraete_bestanden_countdata(){
$this->db->select('geraete.*, orte.name AS ortsname, pruefung.bestanden, pruefung.datum AS letztesdatum, (select count(*) from pruefung as pr where geraete.gid = pr.gid) AS anzahl, pruefer.name as pruefername');
		...
		$this->db->join('pruefer', 'pruefer.pid = pruefung.pid', 'LEFT');
			...
			$this->db->where('bestanden', '0');
	

		return $this->db->count_all_results();
}

function getgeraete_geprueft_countdata(){
$this->db->select('geraete.*, pruefung.bestanden, pruefung.datum AS letztesdatum, (select count(*) from pruefung as pr where geraete.gid = pr.gid) AS anzahl, pruefer.name as pruefername');
		$this->db->from('geraete');
